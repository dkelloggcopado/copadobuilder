...
## Install deploy-only Job Template (Phase 1 — separate Deploy button)

See [`builder_deploy_only.job-template.md`](builder_deploy_only.job-template.md) and [`PREFLIGHT.md`](PREFLIGHT.md).

1. Create Job Template `builder_deploy_only` with **one** step: Function `builder_deploy_package`.
2. Set Copado Builder Settings → **Deploy Job Template** = `builder_deploy_only`.
3. **Commit** uses OOTB `sfdx_commit_1` via **Commit Job Template** (no custom commit template).

## Layout stories (field + profile + page layout)

AI packages often include a **stub** layout (new field only). Deploying that as-is
fails Salesforce validation (`required layout field: Name` / `ParentId`) and would
wipe the real page layout.

`builder_deploy_package` **layout-merge v6** (see script banner in job logs):

1. Stashes the package stub, then retrieves the live org layout (sf crashes if a Layout filename with spaces is already in `force-app`)
2. Falls back to **Metadata API SOAP retrieve** when `sf project retrieve` still fails
3. Merges package custom fields into the live layout
4. Emits `BUILDER_LAYOUT_REMAP|from|to` so Builder can rewrite package + USM before Commit
5. If retrieve/merge fails, **skips** the stub layout and deploys field + profile only

After pulling this repo, **re-paste** `builder-deploy-package/script.sh` into the
Copado Function. Confirm job logs show:

`==> builder_deploy_package layout-merge v6`
